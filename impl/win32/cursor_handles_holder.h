...
class CursorHandlesHolder
{
    ...
    HCURSOR createCursorHandle(EStockCursor cursorKind)
    {
        ...
        if (hCursor==0)
        {
            ...
            {
                return createCursorHandle(EStockCursor::normal); // пытаемся рекурсивно создать обычный курсор
            }
            else
            {
                // чтобы не зарекурсивиться
                throw std::runtime_error("CursorHandlesHolder::createCursorHandle: failed to load stock cursor"); // Даже обычный курсор не получается создать, что-то пошло по пизде
                //return (HCURSOR)0; // нахер не нужны исключения
            }
        }
